...
async function syncVersions() {
  ...
  for (const packagePath of packagePaths) {
    try {
      ...
      if (packageJson.name && packageJson.name.startsWith("@arbiter/")) {
        if (packageJson.version !== targetVersion) {
          console.log(`  ✏️  Updating ${packageJson.name}: ${packageJson.version} → ${targetVersion}`);
...
export function validateVersionConsistency(): boolean {
  ...
  try {
    ...
    for (const packagePath of packagePaths) {
      ...
      if (packageJson.name && packageJson.name.startsWith("@arbiter/")) {
        if (packageJson.version !== targetVersion) {
          console.log(`❌ Version mismatch: ${packageJson.name} has ${packageJson.version}, expected ${targetVersion}`);
          isConsistent = false;
        }
      }
    }
