/**
 * Cloudflare Worker that proxies webhook requests into the Claude Code container
 * Durable Object. Update `ARBITER_HANDLER_IMAGE` in the accompanying wrangler
 * config after publishing a fresh handler image.
 */
...
export default {
  async fetch(request: Request, env: ArbiterEnv, _ctx: ExecutionContext): Promise<Response> {
    const bindingName = env.ARBITER_HANDLER_NAMESPACE || "CLAUDE_CODE_WEBHOOK_CONTAINER";
    ...
    if (!namespace) {
      return new Response(
        `Durable Object namespace "${bindingName}" is not bound in this deployment.`,
        { status: 500 },
      );
    }

    const objectName = env.ARBITER_HANDLER_OBJECT || "claude-code-webhook-container";
    const id = namespace.idFromName(objectName);
    return namespace.get(id).fetch(request);
  },
};

export class ClaudeCodeWebhookContainer {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (this.shouldForwardBody(request.method)) {
      ...
      if (!current.running) {
        ...
        this.state.waitUntil(
          current.monitor().catch((error) => {
            console.error("Claude Code container exited unexpectedly", error);
          }),
        );
      }
