...
/** CUE content indicators for pattern detection */
const CUE_PATTERN_INDICATORS = ["package", "cue", "//"] as const;

/** Minimum content length to consider valid */
const MIN_CONTENT_LENGTH = 10;

if (isBunTestRunner) {
  test.describe.skip("Monaco Editor Functionality", () => {});
} else {
  test.describe("Monaco Editor Functionality", () => {
    ...
    test("should display CUE content in editor", async ({ page }) => {
      ...
      const hasCuePatterns =
        CUE_PATTERN_INDICATORS.some((indicator) => content.includes(indicator)) ||
