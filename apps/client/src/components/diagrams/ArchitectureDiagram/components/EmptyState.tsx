...
const NEXT_STEPS = [
  "Create or import a project with a CUE specification",
  {
    text: "Define services, databases, and other components in your ",
    code: "arbiter.assembly.cue",
  },
  { text: "Run ", code: "arbiter generate", suffix: " to create visualizations" },
  "Return here to see your architecture diagram",
] as const;

/** Render a code snippet with styling */
const CodeSnippet: React.FC<{ children: string }> = ({ children }) => (
  <code className="bg-blue-100 dark:bg-blue-900 px-1 rounded">{children}</code>
);

/** Render a single step item */
const StepItem: React.FC<{ step: (typeof NEXT_STEPS)[number] }> = ({ step }) => {
...
export const EmptyState: React.FC<EmptyStateProps> = ({ className = "" }) => (
  <div className={clsx("text-center py-12 px-6", className)}>
    ...
    <p className="text-gray-600 dark:text-graphite-400 mb-6 max-w-md mx-auto">
      Your project doesn't have any components defined. Get started by defining your architecture in
      CUE.
    </p>
    ...
    <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-5 max-w-lg mx-auto text-left">
      <h4 className="font-medium text-blue-900 dark:text-blue-100 mb-3 flex items-center gap-2">
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={ICON_PATHS.info} />
        </svg>
        Next Steps:
      </h4>
