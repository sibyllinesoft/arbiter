...
import { buildCueDiagramModel, layoutCueDiagram } from "@/utils/diagramTransformers";
...
export const CueDrivenArchitectureDiagram: React.FC<CueDrivenArchitectureDiagramProps> = ({
  ...
  const { components, connections } = useMemo(
    () => buildCueDiagramModel(cueData, diagramType),
    [cueData, diagramType],
  );

  // Apply layout algorithm
  const { components: layoutedComponents, viewport } = useMemo(
    () => layoutCueDiagram(components, connections, layoutType),
