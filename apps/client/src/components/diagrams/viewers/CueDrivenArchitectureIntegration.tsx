...
/**
 * Build architecture data from resolved spec response.
 */
function buildArchitectureData(resolved: any, projectId: string): CueArchitectureData {
  return {
    metadata: {
...
/** Props for the CueDrivenArchitectureIntegration component */
...
/**
 * Integration component that fetches CUE spec data and renders architecture diagrams.
 * Provides diagram type selection, layout options, and handles loading/error states.
 */
export const CueDrivenArchitectureIntegration: React.FC<CueDrivenArchitectureIntegrationProps> = ({
  ...
  const fetchCueData = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      ...
      const architectureData = buildArchitectureData(resolved, projectId);
