...
const ArchitectureDiagram: React.FC<ArchitectureDiagramProps> = ({ projectId, className = '' }) => {
  ...
  const generateComponentsFromData = (
    ...
    const getSourceFile = (itemData: any): string => {
  ...
  // Define components for rendering connections (use hardcoded for now, or dynamic if available)
  const componentsToRender: Component[] = ARCHITECTURE_COMPONENTS; // or dynamicComponents if using dynamic
  ...
  if (projectData) {
    ...
    // Helper to get source file path (same as above)
    const getSourceFile = (data: any): string => {
      if (data.provenance?.evidence && data.provenance.evidence.length > 0) {
        return data.provenance.evidence[0];
      }
      const metadataSource = data.metadata?.sourceFile || data.sourceFile;
      if (metadataSource) {
        return metadataSource;
      }
      // Intelligent fallback based on type
      const artifactType = data.type || data.metadata?.type || 'service';
      const language = data.metadata?.language || '';
      if (artifactType === 'service' && (language === 'docker' || data.containerImage)) {
        return 'docker-compose.yml';
      } else if (
        ['library', 'cli', 'binary', 'frontend', 'service'].includes(artifactType) &&
        (language === 'javascript' || language === 'typescript')
  ...
  const renderConnection = (connection: Connection) => {
    const fromComponent = componentsToRender.find(
      (c: Component) => c.id === connection.from.componentId
    );
    const toComponent = componentsToRender.find(
    ...
    const fromPort = connection.from.portId
      ? fromComponent.ports?.find(
          (p: { id: string; position: { x: number; y: number } }) => p.id === connection.from.portId
        )
      : null;
    const toPort = connection.to.portId
      ? toComponent.ports?.find(
          (p: { id: string; position: { x: number; y: number } }) => p.id === connection.to.portId
        )
      : null;
        ...
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            {error.includes('not found') && (
              <button
                onClick={() => {
                  // Clear the projectId or refresh - parent should handle
