...
describe("Input", () => {
  ...
  describe("variants", () => {
    it.each([["default"], ["error"], ["success"], ["warning"]])(
  ...
  describe("icons", () => {
    ...
    it("adjusts padding when icons are present", () => {
      render(<Input leftIcon={<Search />} rightIcon={<User />} data-testid="icon-input" />);
    ...
    it("adjusts padding for different sizes with icons", () => {
      const { rerender } = render(
        <Input leftIcon={<Search />} size="sm" data-testid="small-icon-input" />,
      );

      expect(screen.getByTestId("small-icon-input")).toHaveClass("pl-8");

      rerender(<Input leftIcon={<Search />} size="lg" data-testid="large-icon-input" />);
