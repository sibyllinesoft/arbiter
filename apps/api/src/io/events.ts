...
import type { AuthContext, Event, EventType, ServerConfig, WebSocketMessage } from "../util/types";
...
export class EventService {
  ...
  private async handleSubscribeAction(
    connectionId: string,
  ...
  private handleUnsubscribeAction(
    connectionId: string,
    projectId: string | undefined,
    channel: string | undefined,
  ): void {
  ...
  private async handleEventMessage(connectionId: string, message: WebSocketMessage): Promise<void> {
    ...
    if (action === "subscribe") {
      await this.handleSubscribeAction(connectionId, projectId, channel);
    } else if (action === "unsubscribe") {
      this.handleUnsubscribeAction(connectionId, projectId, channel);
    } else {
