...
import { buildProjectStructure } from "../git/project-analysis";
import { type FileInfo, FileWorkerPool } from "../scanner/file-scanner-worker";
import type { GitScanResult, GitScannerAdapter } from "../scanner/git-scanner.types";
...
export class GitScanner implements GitScannerAdapter {
  ...
  /** Fallback project name when extraction fails */
  private static readonly DEFAULT_PROJECT_NAME = "imported-project";

  /** URL extraction patterns ordered by specificity */
  private static readonly PROJECT_NAME_EXTRACTORS: Array<(url: string) => string | null> = [
    // HTTPS/HTTP URLs: /owner/repo or /owner/repo.git
    (url) => url.match(/\/([^\/]+?)(?:\.git)?$/)?.[1] ?? null,
    // SSH URLs: git@host:owner/repo or git@host:owner/repo.git
    (url) => url.match(/:(?:[^\/]+\/)?([^\/]+?)(?:\.git)?$/)?.[2] ?? null,
  ...
  private extractProjectNameFromGitUrl(gitUrl: string): string {
    ...
    for (const extractor of GitScanner.PROJECT_NAME_EXTRACTORS) {
      const result = extractor(cleanUrl);
      if (result) return result;
    }

    return GitScanner.DEFAULT_PROJECT_NAME;
  }
  ...
  private async cloneRepository(
    ...
    const authenticatedUrl = this.buildAuthenticatedUrl(gitUrl, authToken);

    try {
      await execFileAsync(
        "git",
        ["clone", "--depth", "1", "--single-branch", authenticatedUrl, targetPath],
        { timeout: this.cloneTimeout, maxBuffer: 10 * 1024 * 1024 },
      );
    } catch (error) {
      throw this.handleCloneError(error);
    }
  }

  /**
   * Build an authenticated URL for git clone.
   */
  private buildAuthenticatedUrl(gitUrl: string, authToken?: string): string {
    if (authToken && gitUrl.startsWith("https://")) {
      return gitUrl.replace("https://", `https://oauth2:${authToken}@`);
    }
    return gitUrl;
  }
  ...
  private isTimeoutError(error: unknown): boolean {
    ...
    return error.message.includes("ETIMEDOUT") || (error as any).killed === true;
  }

  /** Handle clone errors and convert to user-friendly messages */
  private handleCloneError(error: unknown): Error {
    if (this.isTimeoutError(error)) {
      return new Error(
        `Repository clone timed out after ${this.cloneTimeout / 1000}s. ` +
          `The repository may be too large or the network is slow. ` +
          `Set GIT_CLONE_TIMEOUT environment variable to increase the limit.`,
      );
    }
