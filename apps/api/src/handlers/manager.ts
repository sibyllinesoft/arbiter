...
export class CustomHandlerManager {
  ...
  constructor(
    ...
    // Configure discovery - use environment variable or config, default to ./handlers
    const handlersDir =
      process.env.HANDLERS_DIR ||
      this.config.handlers?.directory ||
      join(process.cwd(), "handlers");

    this.handlersDirectory = handlersDir;

    const discoveryConfig: HandlerDiscoveryConfig = {
      handlersDirectory: handlersDir,
    ...
    let r2Adapter: CloudflareR2HandlerAdapter | undefined;
    const cloudflareHandlersConfig = this.config.handlers?.cloudflare;
    const r2Config = cloudflareHandlersConfig?.r2;
    if (r2Config?.binding) {
      r2Adapter = new CloudflareR2HandlerAdapter({
        bindingName: r2Config.binding,
        prefix: r2Config.prefix,
      });
    }

    this.discovery = new HandlerDiscovery(discoveryConfig, this.logger, {
      cloudflareR2Adapter: r2Adapter,
    });
  ...
  async createHandler(options: HandlerCreationOptions): Promise<RegisteredHandler> {
    ...
    const handlersRoot = join(this.handlersDirectory, options.provider);
...
`;
  ...
  async createHandlerStructure(): Promise<void> {
    ...
    const handlersDir =
      process.env.HANDLERS_DIR ||
      this.config.handlers?.directory ||
      path.join(process.cwd(), "handlers");
