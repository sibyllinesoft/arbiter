...
describe("SpecWorkbenchDB", () => {
  ...
  describe("Project Operations", () => {
    ...
    it("should reject duplicate project IDs", async () => {
      ...
      await expect(db.createProject(testProjectId, "Duplicate Project")).rejects.toThrow();
    });
    ...
    it("should throw error when deleting non-existent project", async () => {
      await expect(db.deleteProject("non-existent-id")).rejects.toThrow("Project not found");
    });
  });
  ...
  describe("Fragment Operations", () => {
    ...
    it("should throw error when updating non-existent fragment", async () => {
      await expect(db.updateFragment(testProjectId, "non-existent.cue", "content")).rejects.toThrow(
        "Fragment not found",
      );
    });
    ...
    it("should throw error when deleting non-existent fragment", async () => {
      await expect(db.deleteFragment(testProjectId, "non-existent.cue")).rejects.toThrow(
        "Fragment not found",
      );
    });
  });
  ...
  describe("Version Operations", () => {
    ...
    it("should enforce unique project_id + spec_hash constraint", async () => {
      ...
      await expect(db.createVersion(generateId(), testProjectId, specHash, "{}")).rejects.toThrow();
    });
  ...
  describe("Transaction Handling", () => {
    ...
    it("should prevent duplicate fragments while preserving existing data", async () => {
      await expect(
      ...
      // The original fragment should remain intact
      const fragments = await db.listFragments(testProjectId);
    ...
    it("should handle nested transactions", async () => {
      const result = await db.transaction(async () => {
        ...
        // Nested transaction
        const nestedFragment = await db.transaction(async () => {
          return db.createFragment(generateId(), testProjectId, "test2.cue", "content2");
        });

        return { fragment1, nestedFragment };
      });
  ...
  describe("Performance and Concurrency", () => {
    ...
    it("should maintain performance with many versions", async () => {
      ...
      // Create 100 versions
      for (let i = 0; i < 100; i++) {
        await db.createVersion(generateId(), testProjectId, `hash_${i}`, `{"version": ${i}}`);
      }
