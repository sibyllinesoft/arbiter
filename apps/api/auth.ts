...
import { parseBearerToken, logger } from "./utils.ts";
...
export class AuthService {
  ...
  createProjectAccessMiddleware() {
    return (authContext: AuthContext, projectId: string): { 
      authorized: boolean; 
      response?: Response 
    } => {
   ...
   */
  listTokens(): Array<{ token: string; userId?: string; projectCount: number }> {
