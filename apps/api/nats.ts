 ...
 */
import { connect, NatsConnection, ConnectionOptions, NatsError } from "nats";
import type { NatsConfig, NatsSpecEvent, Event } from "./types.ts";
import { logger, getCurrentTimestamp } from "./utils.ts";
...
export class NatsService {
  ...
  private setupConnectionListeners(): void {
    ...
    this.connection.closed().then((error) => {
      ...
      if (error) {
        logger.error("NATS connection closed with error", error instanceof Error ? error : undefined);
        this.scheduleReconnect();
      } else {
