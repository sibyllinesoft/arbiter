...
describe('StatusBadge', () => {
  ...
  describe('styles', () => {
    ...
    it('renders outlined style correctly', () => {
      const { container } = render(<StatusBadge variant="success" style="outlined">Outlined</StatusBadge>)
      const badge = container.firstChild
      expect(badge).toHaveClass('bg-transparent', 'border-2', 'border-emerald-200', 'text-emerald-700')
    })

    it('renders subtle style correctly', () => {
      const { container } = render(<StatusBadge variant="success" style="subtle">Subtle</StatusBadge>)
      const badge = container.firstChild
      expect(badge).toHaveClass('bg-emerald-50', 'border-transparent', 'text-emerald-700')
    })

    it('fallback to solid style for invalid style', () => {
      const { container } = render(<StatusBadge variant="success" style={'invalid' as any}>Invalid</StatusBadge>)
      const badge = container.firstChild
      expect(badge).toHaveClass('bg-emerald-50', 'border-emerald-200', 'text-emerald-700')
    })
  })
  ...
  describe('dot indicator', () => {
    ...
    it('applies correct dot size for different badge sizes', () => {
      // XS size
      const { container: xsContainer } = render(<StatusBadge size="xs" showDot>XS</StatusBadge>)
      ...
      // MD size
      const { container: mdContainer } = render(<StatusBadge size="md" showDot>MD</StatusBadge>)
      const mdDot = mdContainer.querySelector('.h-2\\.5.w-2\\.5')
      expect(mdDot).toBeInTheDocument()

      // LG size
      const { container: lgContainer } = render(<StatusBadge size="lg" showDot>LG</StatusBadge>)
      const lgDot = lgContainer.querySelector('.h-3.w-3')
      expect(lgDot).toBeInTheDocument()
    })

    it('applies correct dot color based on variant', () => {
      const { container } = render(<StatusBadge variant="success" showDot>Success</StatusBadge>)
      const dot = container.querySelector('.bg-emerald-500')
      expect(dot).toBeInTheDocument()
    })

    it('adds pulse animation when pulse is true', () => {
      const { container } = render(<StatusBadge showDot pulse>Pulsing</StatusBadge>)
      const dot = container.querySelector('.animate-pulse')
      expect(dot).toBeInTheDocument()
    })

    it('does not add pulse animation when pulse is false', () => {
      const { container } = render(<StatusBadge showDot pulse={false}>No Pulse</StatusBadge>)
      const dot = container.querySelector('.animate-pulse')
      expect(dot).not.toBeInTheDocument()
    })
  })
  ...
  describe('icon', () => {
    ...
    it('applies correct icon size for different badge sizes', () => {
      ...
      const { container: xsContainer } = render(
        <StatusBadge size="xs" icon={<CheckCircle />}>XS Icon</StatusBadge>
      )
      ...
      const { container: mdContainer } = render(
        <StatusBadge size="md" icon={<CheckCircle />}>MD Icon</StatusBadge>
      )
      ...
      const { container: lgContainer } = render(
        <StatusBadge size="lg" icon={<CheckCircle />}>LG Icon</StatusBadge>
      )
    ...
    it('applies correct icon color based on variant', () => {
      const { container } = render(
        <StatusBadge variant="error" icon={<XCircle />}>Error Icon</StatusBadge>
      )
  ...
  describe('loading state', () => {
    ...
    it('spinner has correct size for different badge sizes', () => {
      // XS size
      const { container: xsContainer } = render(<StatusBadge size="xs" loading>XS Loading</StatusBadge>)
      ...
      // MD size
      const { container: mdContainer } = render(<StatusBadge size="md" loading>MD Loading</StatusBadge>)
      const mdSpinner = mdContainer.querySelector('.h-2\\.5.w-2\\.5')
      expect(mdSpinner).toBeInTheDocument()

      // LG size
      const { container: lgContainer } = render(<StatusBadge size="lg" loading>LG Loading</StatusBadge>)
  ...
  describe('accessibility', () => {
    ...
    it('maintains proper contrast with different variants', () => {
      const variants: Array<'success' | 'warning' | 'error' | 'info'> = ['success', 'warning', 'error', 'info']
