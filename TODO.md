...
- Rust plugin: cargo manifest, main, config, app state, database, and errors templated via TemplateResolver (`packages/cli/src/templates/rust`).

## Remaining
- Rust plugin: externalize remaining generated assets into `packages/cli/src/templates/rust` (routes, handlers, models, services, tests, Dockerfiles/compose, GitHub Actions) and wire the plugin to use them.
