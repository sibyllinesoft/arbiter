...
export class CLIDocumentationGenerator {
  ...
  private extractTags(command: Command): string[] {
    ...
    if (description.includes("agent") || description.includes("ai")) tags.push("agent-friendly");
    if (
      description.includes("dry-run") ||
      command.options.some((opt) => opt.flags.includes("dry-run"))
    )
      tags.push("safe-mode");
    if (description.includes("watch") || description.includes("live")) tags.push("real-time");
    if (description.includes("generate") || description.includes("create")) tags.push("generative");
    if (description.includes("validate") || description.includes("check")) tags.push("validation");
  ...
  private getCommandHelp(command: Command): string {
    try {
      // Get help text by temporarily capturing help output
  ...
  private renderCategoriesHTML(categorizedCommands: Record<string, ParsedCommandInfo[]>): string {
    ...
    for (const [category, commands] of Object.entries(categorizedCommands)) {
      ...
      for (const command of commands) {
        ...
        if (command.arguments.length > 0) {
          content += '<div class="arguments"><h4>Arguments:</h4>';
          for (const arg of command.arguments) {
        ...
        if (command.options.length > 0) {
          content += '<div class="options"><h4>Options:</h4>';
          for (const option of command.options) {
