...
/**
 * Try to extract a comment from a line.
 */
function tryCollectComment(line: string, ctx: DemoParseContext): boolean {
  if (!line.startsWith("//")) return false;
  ctx.currentComment += line.substring(2).trim() + " ";
  return true;
}
...
/**
 * Create a DemoType from parsed line components.
 */
function createDemoType(
  typeName: string,
  definition: string,
  comment: string,
  fileName: string,
...
/**
 * Try to parse a type definition from a line.
 */
function tryParseTypeDefinition(line: string, lineNumber: number, ctx: DemoParseContext): boolean {
  if (!isTypeDefinition(line)) return false;

  const match = line.match(/^(#\w+):\s*(.+)$/);
  if (match) {
    const [, typeName, definition] = match;
    const type = createDemoType(typeName, definition, ctx.currentComment, ctx.fileName, lineNumber);
    ctx.types.push(type);
  }
...
/**
 * Process a single line during CUE parsing.
 */
function processDemoLine(line: string, lineNumber: number, ctx: DemoParseContext): void {
  const trimmedLine = line.trim();

  if (tryCollectComment(trimmedLine, ctx)) return;
  if (tryParseTypeDefinition(trimmedLine, lineNumber, ctx)) return;

  // Non-comment, non-type lines reset comment buffer
  if (trimmedLine) {
    ctx.currentComment = "";
  }
}
...
function parseSimpleCUE(content: string, fileName: string): DemoType[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    processDemoLine(lines[i], i + 1, ctx);
  }
...
/**
 * Parse result from schema files
 */
...
const SCHEMA_FILES = ["core_types.cue", "app_spec.cue", "feature_spec.cue", "completion_rules.cue"];

/**
 * Initialize demo paths and output directory
 */
async function initializeDemoPaths(): Promise<DemoPaths> {
  ...
  return { projectRoot, schemaDir, outputDir };
}
...
): Promise<{ types: DemoType[]; packageName: string }> {
...
/**
 * Parse all schema files from directory
 */
async function parseAllSchemaFiles(schemaDir: string): Promise<ParseResult> {
  ...
  for (const fileName of SCHEMA_FILES) {
    ...
    try {
      const result = await parseSchemaFile(filePath, fileName, packageName);
      packageName = result.packageName;
      allTypes = allTypes.concat(result.types);
    } catch (error) {
      console.warn(`   ⚠️  Could not parse ${fileName}: ${error}`);
    }
  }

  return { types: allTypes, packageName };
}

/**
 * Log discovered types by kind
 */
function logDiscoveredTypes(types: DemoType[]): void {
  console.log("📋 Discovered types:");
  const typesByKind = types.reduce(
    (acc, type) => {
...
async function writeDocumentation(
  ...
  const markdownContent = generateMarkdownDemo(types, packageName);
  const htmlContent = generateHTMLDemo(types, packageName);
...
/**
 * Log completion message with generated file paths
 */
function logCompletion(markdownPath: string, htmlPath: string): void {
...
async function runDemo() {
  ...
  try {
    const paths = await initializeDemoPaths();
    const { types, packageName } = await parseAllSchemaFiles(paths.schemaDir);

    console.log(`\n✅ Parsed ${types.length} total types from package "${packageName}"\n`);
    logDiscoveredTypes(types);
    ...
    logCompletion(markdownPath, htmlPath);
