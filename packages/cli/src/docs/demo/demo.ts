...
import { CLIDocumentationGenerator } from "@/docs/generator/cli-doc-generator.js";
...
/**
 * Analyze CLI structure and return commands with categories
 */
function analyzeCliStructure(generator: CLIDocumentationGenerator): {
  commands: any[];
...
/**
 * Display sample command analysis
 */
function displaySampleCommands(commands: any[]): void {
...
async function generateDocumentationFiles(
  ...
  const htmlTemplate = templateEngine.getBuiltInTemplate({
    ...
    options: { includeSearch: true, includeToc: true },
  });
...
/**
 * Display summary and file sizes
 */
...
async function main() {
  ...
  try {
    ...
    const { commands, categories } = analyzeCliStructure(generator);
    displaySampleCommands(commands);
    ...
    await generateDocumentationFiles(templateEngine, templateData, demoDir);
    await displaySummary(commands, categories, demoDir);
  } catch (error) {
    console.error(
