...
// Re-export data helpers for external consumers (commands now rely on data.ts directly)
export * from "./data.js";
...
export async function taskCommand(
  ...
  try {
    ...
    switch (action) {
      ...
      default:
        console.error(chalk.red(`Unknown action: ${action}`));
        console.log(chalk.dim("Available actions: list, create, batch, update, complete"));
        return 1;
    }
  } catch (error) {
...
async function listEpics(
  ...
  const structured = options.format === "json";
  return await withOptionalProgress({ text: "Loading epics..." }, structured, async () => {
...
async function showEpic(
  ...
  const structured = options.format === "json";
  return await withOptionalProgress({ text: `Loading epic ${epicId}...` }, structured, async () => {
...
async function showStats(
  ...
  const structured = options.format === "json";
  return await withOptionalProgress({ text: "Calculating statistics..." }, structured, async () => {
...
async function listTasks(
  ...
  const structured = options.format === "json";
  return await withOptionalProgress({ text: "Loading tasks..." }, structured, async () => {
 ...
 */
function withOptionalProgress<T>(
  progressOptions: { text: string; color?: string },
  suppress: boolean,
  operation: () => Promise<T>,
): Promise<T> {
  if (suppress) {
  ...
  return withProgress(progressOptions, operation);
}
