...
/**
 * Generate a single justfile recipe entry.
 */
function generateJustfileRecipe(task: TestTask): string {
  const cwd = task.cwd || ".";
...
/**
 * Generate justfile content from tasks and configuration.
 */
export function generateJustfileContent(
  tasks: TestTask[],
  pm: PackageManagerCommandSet,
  testsWorkspace?: string,
): string {
  let justfile = "# Generated by Arbiter\n\n";
  justfile += "# Default recipe to list available commands\n";
  justfile += "default:\n    @just --list\n\n";
  ...
  for (const task of tasks) {
    justfile += generateJustfileRecipe(task);
  }
...
/**
 * Write justfile and return relative path.
 */
async function writeJustfile(
  outputDir: string,
  outputPath: string,
  content: string,
  options: GenerateOptions,
): Promise<string> {
  const justfilePath = path.join(outputDir, outputPath);
  await ensureDirectory(path.dirname(justfilePath), options);

  await writeFileWithHooks(justfilePath, content, options);

  if (options.verbose) {
    reporter.info(`🧰 Justfile written to ${justfilePath}`);
  }

  const relativePath = path.relative(outputDir, justfilePath).replace(/\\/g, "/");
  return relativePath.length > 0 ? relativePath : path.basename(justfilePath);
}
...
export async function generateMasterTestRunner(
  ...
  const outputPath = masterConfig?.output ?? "justfile";
  const justfileContent = generateJustfileContent(tasks, pm, testsWorkspace);
  const relativePath = await writeJustfile(outputDir, outputPath, justfileContent, options);
  return [relativePath];
}
