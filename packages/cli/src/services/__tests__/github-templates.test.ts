...
import {
  ...
  showTemplate,
  validateTemplates,
} from "../github-templates/index.js";

const baseConfig: any = {
  projectDir: process.cwd(),
  github: {
    templates: {
      epic: { name: "epic", description: "Epic template" },
      task: "task-template",
    },
  },
};
...
describe("github templates service", () => {
  ...
  it("shows template or reports missing", async () => {
    ...
    const ok = await showTemplate(baseConfig.github.templates, "epic", "yaml");
    expect(ok).toBe(0);
  ...
  it("validates template configuration and reports errors", async () => {
    ...
    ).mockResolvedValue([{ field: "epic", message: "invalid" }]);
