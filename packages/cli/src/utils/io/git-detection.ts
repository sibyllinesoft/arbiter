...
type RepositoryResult = { repo: GitHubRepo; source: "config" | "detected" | "merged" };
type SmartRepoOptions = {
  useConfig?: boolean;
  useGitRemote?: boolean;
  verbose?: boolean;
};
 ...
 */
function logVerbose(message: string, verbose: boolean | undefined): void {
  if (verbose) {
...
/**
 * Handle case where Git detection failed
 */
function handleDetectionFailed(
  configRepo: GitHubRepo | undefined,
  error: string | undefined,
  verbose: boolean | undefined,
): RepositoryResult | null {
  logVerbose(`Git detection failed: ${error}`, verbose);

  if (configRepo) {
    logVerbose("Using repository from config file", verbose);
...
/**
 * Handle case where no config repo exists (use detected)
 */
function handleNoConfigRepo(remote: GitRemoteInfo, verbose: boolean | undefined): RepositoryResult {
  logVerbose(`Auto-detected GitHub repository: ${remote.owner}/${remote.repo}`, verbose);
  return {
    repo: { owner: remote.owner, repo: remote.repo },
    source: "detected",
  };
}

/**
 * Handle conflict between config and detected repos
 */
function handleConflict(
  configRepo: GitHubRepo,
  conflict: RepositoryConflict,
  options: SmartRepoOptions,
): RepositoryResult {
  if (options.verbose) {
...
export function getSmartRepositoryConfig(
  ...
  if (!detection.detected) {
    return handleDetectionFailed(configRepo, detection.error, options.verbose);
  }

  // If no config repo, use detected
  if (!configRepo) {
    return handleNoConfigRepo(detection.remote!, options.verbose);
  }
  ...
  if (!conflict) {
    logVerbose(
      `Repository configuration validated: ${configRepo.owner}/${configRepo.repo}`,
      options.verbose,
    );
    return { repo: configRepo, source: "config" };
  }
  ...
  // Handle conflict
  return handleConflict(configRepo, conflict, options);
}
...
/**
 * Find the root directory of the git repository containing the given directory.
