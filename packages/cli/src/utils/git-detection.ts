...
export function getSmartRepositoryConfig(
  ...
  if (!configRepo) {
    if (options.verbose) {
      console.log(chalk.dim(`Auto-detected GitHub repository: ${detection.remote!.owner}/${detection.remote!.repo}`));
    }
    return {
      repo: {
  ...
  if (!conflict) {
    ...
    if (options.verbose) {
      console.log(chalk.dim(`Repository configuration validated: ${configRepo.owner}/${configRepo.repo}`));
    }
    return { repo: configRepo, source: "config" };
  }
 ...
 */
export function createRepositoryConfig(owner: string, repo: string, baseConfig?: Partial<GitHubRepo>): GitHubRepo {
  return {
    owner,
    repo,
...
export function validateRepositoryConfig(repo: GitHubRepo): {
  ...
  if (repo.owner?.includes("/")) {
    ...
    suggestions.push(`Did you mean owner: "${repo.owner.split("/")[0]}", repo: "${repo.owner.split("/")[1]}"?`);
  }
  ...
  if (repo.repo?.endsWith(".git")) {
    suggestions.push(`Repository name should not include .git extension: "${repo.repo.replace(".git", "")}"`);
  }
