...
/** Status-to-icon mapping for step report display */
...
/** Status-to-color mapping for step report display */
const STEP_COLORS: Record<StepStatus, ChalkInstance> = {
...
function formatStepDuration(step: StepInfo): string {
  ...
  if (step.startTime && !step.endTime) {
    return ` (${Date.now() - step.startTime}ms ongoing)`;
  }
  return "";
}

/**
 * Format a single step line for report display.
 */
function formatStepLine(step: StepInfo): string {
...
export class Progress {
  ...
  getStepsReport(): string {
    if (this.totalSteps === 0) return "";
    return `\n${this.steps.map(formatStepLine).join("\n")}`;
  }
}
