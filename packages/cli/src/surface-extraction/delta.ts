...
import type { SurfaceOutput } from '../utils/standardized-output.js';
...
export async function calculateSurfaceDelta(
  ...
  try {
    ...
    const added = surface.symbols.filter(
      symbol => !existingSurface.symbols.some(existing => existing.name === symbol.name && existing.type === symbol.type)
    );

    const removed = existingSurface.symbols.filter(
      symbol => !surface.symbols.some(current => current.name === symbol.name && current.type === symbol.type)
    );
