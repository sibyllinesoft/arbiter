...
export const envelopeBaseSchema = z.object({
  apiVersion: z.string().refine(
    (version) => VERSION_COMPATIBILITY.supported.includes(version) || 
...
export class SchemaVersionValidator {
  ...
  private isMigrationSupported(version: string): boolean {
    return VERSION_COMPATIBILITY.supported.includes(version) ||
           VERSION_COMPATIBILITY.deprecated.includes(version);
  }
   ...
   */
  private validateVersionCompatibility(version: string, operation: 'read' | 'write', operationId?: string): void {
