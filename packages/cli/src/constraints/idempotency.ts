...
export class IdempotencyValidator {
  ...
  async validateRepeatedExecution<TInput, TOutput>(
    ...
    for (let i = 0; i < repetitions; i++) {
      const iterationId = globalConstraintEnforcer.startOperation(
        `${operation}:repeat:${i}`,
        { operationId, iteration: i, totalRepetitions: repetitions }
      );
  ...
  private isTimestampField(fieldName: string): boolean {
    const timestampFields = [
      'timestamp', 'createdAt', 'updatedAt', 'modifiedAt', 'date', 'time',
      'created_at', 'updated_at', 'modified_at', 'processed_at'
    ];
...
export async function withIdempotencyValidation<TInput, TOutput>(
  ...
  return globalIdempotencyValidator.validateIdempotency(
    operation,
    inputs,
    executor,
    options
