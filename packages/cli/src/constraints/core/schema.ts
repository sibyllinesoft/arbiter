...
export class SchemaVersionValidator {
  ...
  private emitVersionViolation(version: string, violation: ConstraintViolationError): never {
    globalConstraintEnforcer.emit("constraint:violation", {
  ...
  private checkUnsupportedVersion(
    version: string,
    operation: "read" | "write",
    operationId?: string,
  ): void {
    if (!VERSION_COMPATIBILITY.unsupported.includes(version)) return;
    ...
    this.emitVersionViolation(version, violation);
  }

  private checkWriteVersionRequirement(
    version: string,
    operation: "read" | "write",
    operationId?: string,
  ): void {
    if (operation !== "write" || version === LATEST_API_VERSION) return;
  ...
  private warnDeprecatedVersion(version: string, operationId?: string): void {
    if (!VERSION_COMPATIBILITY.deprecated.includes(version)) return;
  ...
  private validateVersionCompatibility(
    ...
    this.checkUnsupportedVersion(version, operation, operationId);
    this.checkWriteVersionRequirement(version, operation, operationId);
    this.warnDeprecatedVersion(version, operationId);
  }
