...
/**
 * Default constraint values from docs/ARCHITECTURE_REVIEW.md section 13
 */
...
export class ConstraintEnforcer extends EventEmitter {
  ...
  /**
   * Check if two values are both non-null objects of the same type
   */
  private areComparableObjects(a: unknown, b: unknown): boolean {
    if (a === b) return false; // Already equal, no need for object comparison
   ...
   */
  private objectKeysEqual(aObj: Record<string, unknown>, bObj: Record<string, unknown>): boolean {
  ...
  private deepEqual(a: unknown, b: unknown): boolean {
    ...
    if (!this.areComparableObjects(a, b)) return false;

    return this.objectKeysEqual(a as Record<string, unknown>, b as Record<string, unknown>);
  }
}
