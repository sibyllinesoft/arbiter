...
export class ConstraintSystem extends EventEmitter {
  ...
  async executeWithConstraints<T>(
    ...
    return constrainedOperation(operation, async () => {
  ...
  getSystemStatus(): ConstraintSystemStatus {
    const constraintStatus = globalConstraintEnforcer.getConstraintStatus();
  ...
  generateComplianceReport(): string {
    ...
    lines.push(`  Rate Limit: ${status.constraints.rateLimit.requests} req/${status.constraints.rateLimit.windowMs}ms`);
    ...
    lines.push(`  Sandbox: ${status.sandbox.activeOperations} active ops, ${status.sandbox.complianceRate.toFixed(1)}% compliant`);
    lines.push(`  File System: ${status.fileSystem.symlinks} symlinks, ${status.fileSystem.invalidPaths} invalid paths`);
    lines.push(`  Idempotency: ${status.idempotency.cacheSize} cached, ${status.idempotency.validations} validated`);
    lines.push(`  Schema: version ${status.schema.latestVersion}, ${status.schema.deprecatedWarnings} warnings`);
  ...
  private setupViolationTracking(): void {
    ...
    this.sandboxValidator.on?.('constraint:violation', (event) => {
      ...
      if (count > 0) {
        switch (constraint) {
          ...
          case 'rateLimit':
            ...
            break;
          case 'sandboxCompliance':
            suggestions.push('Ensure all analyze/validate operations use API client instead of direct tools');
            break;
          case 'symlinkPrevention':
...
export function getGlobalConstraintSystem(): ConstraintSystem {
  if (!globalConstraintSystem) {
    throw new Error('Constraint system not initialized. Call initializeGlobalConstraintSystem first.');
  }
  return globalConstraintSystem;
}