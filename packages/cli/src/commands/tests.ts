...
async function parseInvariants(assemblyPath: string): Promise<Invariant[]> {
  try {
    ...
    throw new Error(`Failed to parse invariants: ${error instanceof Error ? error.message : String(error)}`);
  }
}
 ...
 */
function generateTestTemplate(invariant: Invariant, language: keyof typeof LANGUAGE_CONFIGS): TestTemplate {
 ...
 */
async function checkExistingTests(testTemplates: TestTemplate[], outputDir: string): Promise<string[]> {
  ...
  for (const template of testTemplates) {
    ...
    try {
      ...
      if (content.includes(`ARBITER_INVARIANT_${template.invariantName.toUpperCase().replace(/[^A-Z0-9]/g, '_')}`)) {
 ...
 */
async function analyzeCoverage(invariants: Invariant[], testPaths: string[]): Promise<CoverageReport> {
  const timestamp = new Date().toISOString();
  const testFiles: CoverageReport['test_files'] = [];
  const invariantCoverage: Map<string, { covered: boolean; test_files: string[]; test_count: number }> = new Map();
...
function generateJUnitXML(report: CoverageReport): string {
  const testcases = report.invariants.map(inv => `
    <testcase 
 ...
 */
export async function scaffoldCommand(options: TestsOptions, config: CLIConfig): Promise<number> {
  try {
 ...
 */
export async function coverCommand(options: TestsOptions, config: CLIConfig): Promise<number> {
  try {
    ...
    console.log(`  Coverage ratio: ${report.coverage_ratio >= threshold ? chalk.green : chalk.red}${(report.coverage_ratio * 100).toFixed(1)}%`);
    ...
    if (options.verbose) {
      ...
      for (const inv of report.invariants) {
        ...
        if (inv.covered && inv.test_files.length > 0) {
          console.log(`      ${chalk.dim(`Tests: ${inv.test_files.map(f => path.relative(process.cwd(), f)).join(', ')}`)}`);
        }
      }
    ...
    const reportPath = path.isAbsolute(reportFilename) ? reportFilename : path.join(outputDir, reportFilename);
    ...
    if (options.junit) {
      ...
      const junitPath = path.isAbsolute(options.junit) ? options.junit : path.join(outputDir, options.junit);
