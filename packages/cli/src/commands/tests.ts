...
type SupportedTestLanguage = keyof typeof LANGUAGE_CONFIGS;
...
export async function scaffoldCommand(options: TestsOptions, _config: CLIConfig): Promise<number> {
  try {
    ...
    const results = await executeTestFileGeneration(
      testTemplates,
      outputDir,
      existingTests,
      options.force,
...
function prepareOutputDirectory(options: TestsOptions, language: SupportedTestLanguage): string {
  const outputDir = (options as any).outputDir ?? options.output;
  return outputDir || getDefaultTestDir(language);
}
