 ...
 */
export async function examplesCommand(type: string, options: ExamplesOptions, config: Config): Promise<number> {
  try {
    ...
    if (options.profile) {
      templatesToGenerate = filteredTemplates.filter(t => 
        t.name === options.profile || t.tags.includes(options.profile)
      );
    }
    
    if (options.language) {
      templatesToGenerate = filteredTemplates.filter(t =>
        t.tags.includes(options.language)
      );
    }
    ...
    console.error(chalk.red('Example generation failed:'), error instanceof Error ? error.message : String(error));
    return 2;
  }
}
...
function getRustLibraryCargoToml(): string { return `[package]\nname = "example-lib"\nversion = "0.1.0"\nedition = "2021"\n\n[dependencies]\n`; }
function getRustLibrarySrc(): string { return `//! Example Rust library\n\npub fn hello() -> String {\n    "Hello, World!".to_string()\n}\n`; }
function getRustLibraryTests(): string { return `use example_lib::hello;\n\n#[test]\nfn test_hello() {\n    assert_eq!(hello(), "Hello, World!");\n}\n`; }
function getRustLibraryAssemblyCue(): string { return `// Rust Library Assembly\nArtifact: { kind: "library", language: "rust" }\n`; }
function getRustLibraryReadme(): string { return `# Example Rust Library\n\nRust library with zero-cost abstractions.\n`; }

function getGoServiceGoMod(): string { return `module example-service\n\ngo 1.21\n\nrequire github.com/gin-gonic/gin v1.9.0\n`; }
function getGoServiceMain(): string { return `package main\n\nimport "github.com/gin-gonic/gin"\n\nfunc main() {\n\tr := gin.Default()\n\tr.GET("/health", func(c *gin.Context) {\n\t\tc.JSON(200, gin.H{"status": "healthy"})\n\t})\n\tr.Run(":8080")\n}\n`; }
function getGoServiceHandlers(): string { return `package handlers\n\nimport "github.com/gin-gonic/gin"\n\nfunc Health(c *gin.Context) {\n\tc.JSON(200, gin.H{"status": "healthy"})\n}\n`; }
function getGoServiceConfig(): string { return `package config\n\ntype Config struct {\n\tPort string\n}\n`; }
function getGoServiceAssemblyCue(): string { return `// Go Service Assembly\nArtifact: { kind: "service", language: "go" }\n`; }
function getGoServiceReadme(): string { return `# Example Go Service\n\nGo microservice with concurrency patterns.\n`; }
...
function getMonorepoCorePackageJson(): string { return `{"name": "@example/core", "version": "0.1.0"}`; }
function getMonorepoCliPackageJson(): string { return `{"name": "@example/cli", "version": "0.1.0"}`; }
function getMonorepoWebPackageJson(): string { return `{"name": "@example/web", "version": "0.1.0"}`; }
function getMonorepoAssemblyCue(): string { return `// Monorepo Assembly\nArtifact: { kind: "monorepo", language: "typescript" }\n`; }
function getMonorepoReadme(): string { return `# Example Monorepo\n\nMulti-package TypeScript monorepo.\n`; }
