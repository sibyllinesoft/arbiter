...
export async function versionPlanCommand(
  ...
  try {
    ...
    // Load current and previous surfaces
    ...
    // Write plan to file
    const outputPath = options.output || 'version_plan.json';
    await writeFile(outputPath, JSON.stringify(plan, null, 2));

    // Display results
    console.log(chalk.green(`✅ Version plan generated: ${outputPath}`));
    ...
    console.error(chalk.red('❌ Version plan failed:'), error instanceof Error ? error.message : String(error));
    return 1;
  }
}
...
export async function versionReleaseCommand(
  ...
  try {
    ...
    manifests.forEach(manifest => {
      const versionInfo = manifest.current_version ? ` (v${manifest.current_version})` : ' (version not found)';
    ...
    if (options.version) {
      ...
      targetVersion = calculateNextVersion(manifests[0].current_version || '0.0.0', plan.required_bump);
    ...
    if (plan) {
      ...
      console.log(chalk.green(`📝 Changelog ${isDryRun ? 'preview' : 'updated'}: ${changelogPath}`));
    }
    ...
    console.error(chalk.red('❌ Version release failed:'), error instanceof Error ? error.message : String(error));
    return 1;
  }
}
...
async function analyzeAPIChanges(
  ...
  for (const symbol of current.symbols) {
    ...
    if (prevSymbol && prevSymbol.signature !== symbol.signature) {
      ...
      const isBreaking = isSignatureChangeBreaking(prevSymbol.signature, symbol.signature, symbol.type);
...
function parseFunctionSignature(signature: string): {
  ...
  const parameters = paramsStr.split(',').map(param => {
    ...
    const name = colonIndex > -1 ? trimmed.substring(0, colonIndex).replace('?', '').trim() : trimmed;
    const type = colonIndex > -1 ? trimmed.substring(colonIndex + 1).split('=')[0].trim() : 'any';
    
    return { name, type, optional, hasDefault };
  }).filter(Boolean) as Array<{name: string; type: string; optional: boolean; hasDefault: boolean}>;
...
function analyzeParameterChanges(
  ...
  for (let i = 0; i < oldParams.length; i++) {
    ...
    if (isTypeNarrowing(oldParam.type, newParam.type)) {
      return { isBreaking: true, reason: `Parameter '${oldParam.name}' type narrowed from '${oldParam.type}' to '${newParam.type}'` };
    }
 ...
 */
async function extractVersionTraceability(changes: APIChange[]): Promise<VersionTraceability | undefined> {
  try {
    console.log(chalk.dim('  🔗 Extracting traceability information...'));
    
    // Load traceability data
    const requirements = await traceabilityLib.extractRequirements();
    const specifications = await traceabilityLib.extractSpecifications();
    const tests = await traceabilityLib.extractTests();
    const codeArtifacts = await traceabilityLib.extractCodeArtifacts();
    
    // Build traceability graph
    const graph = traceabilityLib.buildTraceabilityGraph(requirements, specifications, tests, codeArtifacts);
    ...
    for (const change of changes) {
      const changeSignature = (change.oldSignature || change.newSignature || change.description).toLowerCase();
      
      // Find artifacts that reference this change
      for (const node of graph.nodes) {
        if (node.content?.toLowerCase().includes(changeSignature) || 
      ...
      const matchingLinks = graph.links.filter(link => {
        ...
        return (sourceNode?.content?.toLowerCase().includes(changeSignature) ||
    ...
    console.log(chalk.dim(`    Found ${traceability.coverage.requirements_linked} requirements, ${traceability.coverage.specifications_linked} specs, ${traceability.coverage.tests_linked} tests, ${traceability.coverage.code_linked} code artifacts`));
    
    return traceability;
    
  } catch (error) {
    console.warn(chalk.yellow(`⚠️  Could not extract traceability information: ${error instanceof Error ? error.message : String(error)}`));
    return undefined;
  }
}
...
async function generateVersionPlan(
  ...
  if (newFeatures.length > 5) {
    recommendations.push('Large number of new features - consider splitting into multiple releases');
  }
  ...
  if (traceabilityInfo) {
    if (traceabilityInfo.coverage.requirements_linked === 0) {
      recommendations.push('Consider linking changes to specific requirements for better traceability');
    }
    
    if (traceabilityInfo.coverage.tests_linked === 0 && (breakingChanges.length > 0 || newFeatures.length > 0)) {
      recommendations.push('Add tests to validate the changes and improve traceability');
    }
    
    if (traceabilityInfo.requirements.length > 0) {
      recommendations.push(`Changes linked to ${traceabilityInfo.requirements.length} requirement(s) - ensure all are addressed`);
    }
  }
...
function displayVersionPlan(plan: VersionPlan, verbose?: boolean): void {
  ...
  console.log(`  Required bump: ${chalk.bold(getBumpColor(plan.required_bump)(plan.required_bump))}`);
  ...
  if (plan.traceability) {
    ...
    if (verbose && trace.requirements.length > 0) {
      ...
      trace.requirements.slice(0, 3).forEach(req => {
        console.log(`  • ${chalk.bold(req.id)}: ${req.content.substring(0, 80)}${req.content.length > 80 ? '...' : ''}`);
...
async function generateChangelog(
  ...
  if (plan.traceability && (plan.traceability.requirements.length > 0 || plan.traceability.tests.length > 0)) {
    ...
    if (plan.traceability.requirements.length > 0) {
      ...
      plan.traceability.requirements.slice(0, 5).forEach(req => {
        const shortContent = req.content.length > 60 ? req.content.substring(0, 60) + '...' : req.content;
