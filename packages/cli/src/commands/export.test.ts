  ...
  schemas: {
    ...
    CreateUserRequest: {
      ...
      const proc = spawn({
        cmd: cliPath
          .split(" ")
          .concat(["export", cueFile, "--format", "openapi", "--output", outputFile, "--verbose"]),
        cwd: process.cwd(),
        stdout: "pipe",
        stderr: "pipe",
      });
...
updateUserRequest: #UpdateUserRequest`;
      ...
      const proc = spawn({
        cmd: cliPath
          .split(" ")
          .concat(["export", cueFile, "--format", "types", "--output", outputFile]),
        cwd: process.cwd(),
        stdout: "pipe",
        stderr: "pipe",
      });
  ...
  data: {
    "app.yaml": """
      ...
      const proc = spawn({
        cmd: cliPath
          .split(" ")
          .concat(["export", cueFile, "--format", "k8s", "--output", outputFile]),
        cwd: process.cwd(),
        stdout: "pipe",
        stderr: "pipe",
      });
...
data: {
  key: "value"
      ...
      const proc = spawn({
        cmd: cliPath.split(" ").concat(["export", cueFile, "--format", "openapi"]),
        cwd: process.cwd(),
        stdout: "pipe",
        stderr: "pipe",
      });
...
invalid syntax here {{{`;
    ...
    test("should handle non-existent files", async () => {
      const proc = spawn({
        cmd: cliPath.split(" ").concat(["export", "non-existent-file.cue", "--format", "openapi"]),
        cwd: process.cwd(),
        stdout: "pipe",
        stderr: "pipe",
      });
