...
export async function watchCommand(
  ...
  try {
    ...
    if (agentMode) {
      ...
      console.error(chalk.red('❌ Watch command failed:'), error instanceof Error ? error.message : String(error));
    }
    outputManager.close();
    return 1;
  }
...
async function processBatch(
  ...
  const assemblyChanged = batch.events.some(e => 
    e.path.includes('arbiter.assembly.cue') && e.type !== 'unlink'
  );
  ...
  const codeFiles = otherFiles.filter(f => 
    /\.(ts|js|py|rs|go)$/.test(f)
  );
...
async function validateFiles(
  ...
  for (const file of files) {
    try {
      ...
      if (!agentMode) {
        console.log(chalk.red(`  ❌ ${file} - Read error: ${error instanceof Error ? error.message : String(error)}`));
      }
    }
  }
