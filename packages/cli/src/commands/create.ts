...
async function interactiveSchemaConfig(): Promise<SchemaConfig> {
  ...
  console.log(chalk.dim('Answer a few questions to generate a tailored CUE schema with constraints\n'));
    ...
    const thresholds = await inquirer.prompt([
      {
        ...
        default: 80,
        validate: (input: number) => (input >= 0 && input <= 100) || 'Coverage must be between 0 and 100',
      },
      {
        type: 'input',
 ...
 */
export async function createCommand(type: string = 'schema', options: CreateOptions = {}): Promise<number> {
  if (type !== 'schema') {
  ...
  try {
    ...
    console.error(chalk.red('Error creating schema:'), error instanceof Error ? error.message : String(error));
    return 1;
  }
}