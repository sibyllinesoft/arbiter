 ...
 */
function getRegistryPath(global: boolean = false): string {
  if (global) {
 ...
 */
function getRegistryDir(global: boolean = false): string {
 ...
 */
async function loadImportRegistry(global: boolean = false): Promise<ImportRegistry> {
 ...
 */
async function saveImportRegistry(registry: ImportRegistry, global: boolean = false): Promise<void> {
  const registryPath = getRegistryPath(global);
...
export async function listImports(options: ImportOptions = {}): Promise<number> {
  try {
    ...
    console.error(chalk.red('Error listing imports:'), error instanceof Error ? error.message : String(error));
    return 1;
  }
}
...
export async function addImport(pattern: string, options: ImportOptions = {}): Promise<number> {
  try {
    ...
    if (registry.blocked_imports[parsed.name]) {
      console.error(chalk.red(`Import '${parsed.name}' is blocked: ${registry.blocked_imports[parsed.name].reason}`));
      return 1;
    }
    ...
    console.error(chalk.red('Error adding import:'), error instanceof Error ? error.message : String(error));
    return 1;
  }
}
...
export async function removeImport(pattern: string, options: ImportOptions = {}): Promise<number> {
  try {
    ...
    console.error(chalk.red('Error removing import:'), error instanceof Error ? error.message : String(error));
    return 1;
  }
}
 ...
 */
export async function blockImport(pattern: string, reason: string, options: ImportOptions = {}): Promise<number> {
  try {
    const registry = await loadImportRegistry(options.global);
    const parsed = parseImportPattern(pattern);
    const currentUser = process.env.USER || process.env.USERNAME || 'unknown';
    ...
    console.error(chalk.red('Error blocking import:'), error instanceof Error ? error.message : String(error));
    return 1;
  }
}
 ...
 */
export async function validateImports(files: string[], options: ImportOptions = {}): Promise<number> {
  try {
    const registry = await loadImportRegistry(options.global);
    let hasErrors = false;
    ...
    console.error(chalk.red('Error validating imports:'), error instanceof Error ? error.message : String(error));
    return 1;
  }
}
...
export async function initImportRegistry(options: ImportOptions = {}): Promise<number> {
  try {
    ...
    console.error(chalk.red('Error initializing import registry:'), error instanceof Error ? error.message : String(error));
    return 1;
  }
}
 ...
 */
export async function importCommand(action: string, pattern?: string, options: ImportOptions = {}): Promise<number> {
  switch (action) {
    case 'init':
