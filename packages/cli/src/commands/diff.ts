 ...
 */
function compareSchemas(oldStructure: Map<string, string>, newStructure: Map<string, string>): SchemaChange[] {
...
function getCategoryFromKey(key: string): SchemaChange['category'] {
  ...
  if (key.includes('validation') || key.includes('rules') || key.includes('limit')) return 'constraint';
  return 'field';
}
 ...
 */
function getRemovalImpact(key: string, value: string): SchemaChange['impact'] {
 ...
 */
function getModificationImpact(key: string, oldValue: string, newValue: string): SchemaChange['impact'] {
...
function extractNumericValue(value: string): number | null {
  ...
  return numMatch ? parseFloat(numMatch[2]) : null;
}
...
function getAdditionMigrationHint(key: string, value: string): string {
 ...
 */
export async function diffCommand(oldFile: string, newFile: string, options: DiffOptions = {}): Promise<number> {
  try {
    ...
    try {
      ...
      console.error(chalk.red('File not found:'), error instanceof Error ? error.message : String(error));
      return 1;
    }
    ...
    console.error(chalk.red('Error comparing schemas:'), error instanceof Error ? error.message : String(error));
    return 1;
  }
}