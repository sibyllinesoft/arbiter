...
import type { IDEOptions, CLIConfig } from '../types.js';
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          if (['node_modules', '.git', 'dist', 'build', 'target', '.next', '__pycache__'].includes(entry.name)) {
            continue;
          }
          ...
          else if (['.ts', '.tsx', '.js', '.jsx'].includes(ext) || ['package.json', 'tsconfig.json'].includes(basename)) {
 ...
 */
async function generateVSCodeConfig(languages: ProjectLanguage[], projectPath: string, force: boolean, outputDir?: string): Promise<void> {
    ...
    tasks: [
      ...
      },
      {
        ...
        problemMatcher: {
          ...
          pattern: [
            {
              regexp: '^\\s*"file":\\s*"([^"]+)",\\s*"line":\\s*(\\d+),\\s*"column":\\s*(\\d+),\\s*"severity":\\s*"([^"]+)",\\s*"message":\\s*"([^"]+)"',
              file: 1,
              line: 2,
              column: 3,
              severity: 4,
 ...
 */
async function generateIDEAConfig(languages: ProjectLanguage[], projectPath: string, force: boolean, outputDir?: string): Promise<void> {
...
<project version="4">
  <component name="ProjectTasksOptions">
    ...
    if (force || !(await fs.access(watchersPath).then(() => true, () => false))) {
 ...
 */
async function generateVimConfig(languages: ProjectLanguage[], projectPath: string, force: boolean, outputDir?: string): Promise<void> {
...
`;
  ...
  if (force || !(await fs.access(vimrcPath).then(() => true, () => false))) {
    await fs.writeFile(vimrcPath, vimConfig);
    console.log(chalk.green(`✅ Generated ${vimrcPath}`));
    console.log(chalk.dim('   Add "source .vimrc.local" to your ~/.vimrc to use this configuration'));
  } else {
 ...
 */
export async function ideCommand(options: IDEOptions, config: CLIConfig): Promise<number> {
  try {
    ...
    if (options.detect) {
      ...
      for (const lang of languages) {
        console.log(`${chalk.bold(lang.name)}: ${chalk.green('✓')} (${lang.files.slice(0, 3).join(', ')}${lang.files.length > 3 ? '...' : ''})`);
      }
      return 0;
    }
    ...
    console.error(chalk.red('❌ IDE configuration failed:'), error instanceof Error ? error.message : String(error));
    return 1;
  }
}