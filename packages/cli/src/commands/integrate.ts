...
import type { IntegrateOptions, CLIConfig } from '../types.js';
 ...
 */
async function readAssemblyFile(projectPath: string): Promise<{ buildMatrix?: BuildMatrix; language?: string } | null> {
...
function getDefaultBuildMatrixForProfile(profileType: string, language: string): BuildMatrix {
  ...
  switch (language) {
    ...
    case 'javascript':
      ...
      break;
    case 'python':
      baseMatrix.versions = profileType === 'library' ? ['3.9', '3.10', '3.11', '3.12'] : ['3.11', '3.12'];
      break;
    case 'rust':
...
async function detectProjectInfo(projectPath: string): Promise<ProjectLanguage[]> {
  ...
  try {
    ...
    languages.push({
      ...
      detected: true,
      files: ['package.json'],
      framework: pkg.dependencies?.['next'] ? 'next' : 
                pkg.dependencies?.['react'] ? 'react' :
                pkg.dependencies?.['vue'] ? 'vue' : 'node'
    });
  } catch {
 ...
 */
function generateGitHubMainWorkflow(languages: ProjectLanguage[], matrix?: BuildMatrix) {
  const workflow = {
    name: 'Main Branch',
    on: {
      push: {
 ...
 */
export async function integrateCommand(options: IntegrateOptions, config: CLIConfig): Promise<number> {
  try {
    ...
    if (type === 'pr' || type === 'all') {
      ...
      if (force || !(await fs.access(prPath).then(() => true, () => false))) {
    ...
    if (type === 'main' || type === 'all') {
      ...
      if (force || !(await fs.access(mainPath).then(() => true, () => false))) {
