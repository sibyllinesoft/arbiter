 ...
 */
async function findCueFilesConstrained(patterns: string[], options: {
  recursive: boolean;
  cwd: string;
}): Promise<string[]> {
  const constraintSystem = getGlobalConstraintSystem();
      ...
      for (const chunk of chunks) {
        ...
        for (const file of chunk) {
          try {
            ...
            if (error instanceof ConstraintViolationError) {
              console.error(chalk.red(`Constraint violation in ${path.basename(file)}:`), error.message);
...
async function validateFileConstrained(
  ...
  try {
    ...
    const validatedResponse = constraintSystem.validateApiResponse(validationResult);
    ...
    if (processingTime > 750) {
      console.warn(chalk.yellow(`Warning: ${path.basename(filePath)} took ${processingTime}ms (>750ms constraint)`));
    }
...
function displayResultsConstrained(
  ...
  console.log(chalk.dim(`Performance: avg ${Math.round(avgTime)}ms, max ${Math.round(maxTime)}ms, total ${Math.round(totalTime)}ms`));
