...
import { 
  ...
  CURRENT_VERSIONS 
} from '@arbiter/shared/version';
import { 
  getAvailableMigrationPaths,
  hasMigrationPath,
  estimateMigrationDuration 
} from '@arbiter/shared/migration';
 ...
 */
export const CompatCheckOptionsSchema = z.object({
...
export const MigrationOptionsSchema = z.object({
  ...
  component: z.enum(['api_version', 'schema_version', 'contract_version', 'ticket_format']).describe('Component to migrate'),
...
function outputCompatibilityText(result: CompatibilityResult, verbose: boolean): void {
  ...
  if (result.version_mismatches.length > 0) {
    ...
    for (const mismatch of result.version_mismatches) {
      ...
      console.log(`  ${icon} ${mismatch.component}: expected ${mismatch.expected}, got ${mismatch.actual}`);
    }
    console.log();
  }
...
function outputCompatibilityTable(result: CompatibilityResult): void {
  ...
  console.table(result.version_mismatches.map(mismatch => ({
  ...
  for (const [component, sourceVersion] of Object.entries(versionsToCheck)) {
    ...
    if (sourceVersion && sourceVersion !== targetVersion) {
      ...
      if (directPath) {
        ...
        console.log(`  🔧 Command: arbiter migrate --component ${component} --from ${sourceVersion} --to ${targetVersion}`);
      } else {
...
export async function runMigration(options: MigrationOptions): Promise<void> {
  try {
    ...
    if (!hasMigrationPath(component, options.from, options.to)) {
      console.error(`❌ No migration path available: ${component} ${options.from} -> ${options.to}`);
