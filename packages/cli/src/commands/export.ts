...
import type { ExportOptions, CLIConfig, ExportFormat } from '../types.js';
...
export async function exportCommand(
  ...
  try {
    ...
    console.error(chalk.red('Export command failed:'), error instanceof Error ? error.message : String(error));
    return 2;
  }
}
...
async function exportToFormat(
  ...
  return withProgress(
    { text: `Exporting to ${format} format...`, color: 'green' },
    async () => {
      ...
      if (result.files && result.files.length > 0) {
        ...
        if (result.metadata && options.verbose) {
          ...
          console.log(chalk.gray(`  Exported schemas: ${result.metadata.exportedSchemas.join(', ')}`));
        }
      } else {
  ...
  try {
    ...
    if (!result.success || !result.data) {
      ...
      const fallbackFormats = [
        { format: 'openapi', description: 'OpenAPI 3.1 specification', example: '// #OpenAPI api-v1' },
        { format: 'types', description: 'TypeScript type definitions', example: '// #TypeScript models' },
        { format: 'k8s', description: 'Kubernetes YAML manifests', example: '// #K8s deployment' },
        { format: 'terraform', description: 'Terraform HCL configuration', example: '// #Terraform infrastructure' },
        { format: 'json-schema', description: 'JSON Schema specification', example: '// #JsonSchema validation' },
      ];
    ...
    console.log(chalk.gray('Add comments like "// #OpenAPI api-v1" to enable export for that format.'));
 ...
 */
function printFormats(formats: Array<{ format: string; description: string; example: string }>): void {
