...
async function createBackups(files: string[]): Promise<Map<string, string>> {
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.error(chalk.red(`Failed to backup ${file}:`), error instanceof Error ? error.message : String(error));
    }
  }
 ...
 */
async function applyAutomaticMigrations(file: string): Promise<{ success: boolean; errors: string[] }> {
  const errors: string[] = [];
 ...
 */
async function generateMigrationPlan(files: string[], options: MigrateOptions): Promise<MigrationPlan> {
  ...
  for (const file of files) {
    ...
    try {
      ...
      fileInfo.errors.push(`Could not read file: ${error instanceof Error ? error.message : String(error)}`);
      plan.safe_to_proceed = false;
    }
  ...
  for (const fileInfo of filesToMigrate) {
    ...
    if (result.success) {
      ...
      if (validation.valid) {
        ...
        if (backups && backups.has(fileInfo.path)) {
 ...
 */
export async function migrateCommand(patterns: string[], options: MigrateOptions = {}): Promise<number> {
  try {
