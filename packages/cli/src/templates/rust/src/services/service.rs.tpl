...
{{ctor_fields}}
    ...
    pub async fn create(&self, payload: Create{{struct_name}}Request) -> Result<{{struct_name}}, AppError> {
        info!("Creating new {{resource_name}}");
        {{create_body}}
    }

    /// Update {{resource_name}} by ID
    pub async fn update(&self, id: Uuid, payload: Update{{struct_name}}Request) -> Result<{{struct_name}}, AppError> {
