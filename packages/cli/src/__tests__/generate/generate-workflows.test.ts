...
import * as cueUtils from "@/cue/index.js";
import { DEFAULT_PROJECT_STRUCTURE } from "@/io/config/config.js";
import { generateCommand } from "@/services/generate/io/index.js";
...
const runSlow = process.env.ARBITER_RUN_SLOW_TESTS === "1";
const describeGenerate = runSlow ? describe : describe.skip;

describeGenerate("Generate command workflows", () => {
  ...
  beforeEach(async () => {
    // Stub CUE ops to avoid invoking external cue binary during tests
    formatSpy = spyOn(cueUtils, "formatCUE").mockImplementation(async (content: string) => content);
    validateSpy = spyOn(cueUtils, "validateCUE").mockResolvedValue({
      valid: true,
      errors: [],
    } as any);
