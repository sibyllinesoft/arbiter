...
import { validateSpecification, formatWarnings } from "../warnings.js";
...
function generateRandomSpecs(count: number): any[] {
  ...
  for (let i = 0; i < count; i++) {
    specs.push({
      ...
      observability: Math.random() > 0.8 ? {
        logging: { level: "info" },
        monitoring: { metrics: ["response_time"] }
      } : undefined,
      
      environments: Math.random() > 0.8 ? {
