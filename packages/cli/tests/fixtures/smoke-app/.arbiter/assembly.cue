...

{
	...
	"checkout.observability": {
		owner:       "analytics@sibylline.dev"
		description: "Emit structured checkout metrics for upgrade funnel alerts and dashboards."
		kind:        "service"
	}
}
	flows: [
		...
		{
			id: "storefront:enterprise-smoke"
			steps: [
				{
					visit: "/"
				},
				...
				{
					click: "[data-testid=\"plan-enterprise\"]"
				},
				{
					expect_api: {
						method: "POST"
				...
				{
					expect: {
						locator: "[data-testid=\"checkout-success\"]"
						state:   "visible"
					}
				},
			]
	...
	tests: [
		...
		},
		{
			...
			cases: [
				{
					name: "formats enterprise tiers"
					assertions: [
						"Enterprise plan response sorts tiers by monthly price",
						"Upgrade CTA copy includes telemetry payload",
					]
				},
			]
		},
		...
		},
		{
			...
			cases: [
				{
					name: "enterprise upgrade emits telemetry"
					assertions: [
						"Enterprise banner visible after upgrade",
						"Analytics event checkout.enterprise.success emitted with trace id",
						"Stripe webhook ack logged before Lago sync",
					]
				},
			]
		},
