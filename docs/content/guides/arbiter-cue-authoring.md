...
4. Declare services in `services.<name>` with `language`, `serviceType`/`workload`, and grouped dependencies (`dependencies.services`, `dependencies.databases`, etc.).
5. Use `deployments.<environment>` for runtime settings; the legacy singular `deployment` key is no longer supported.
