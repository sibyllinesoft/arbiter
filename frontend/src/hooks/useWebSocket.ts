...
import { wsService, type WebSocketEventHandler } from '../services/websocket';
...
export function useWebSocket(projectId: string | null, options: UseWebSocketOptions = {}) {
  ...
  const handleFragmentUpdated = useCallback((data: WsFragmentUpdatedData) => {
  ...
  const handleResolvedUpdated = useCallback((data: WsResolvedUpdatedData) => {
    dispatch({ 
      type: 'SET_RESOLVED', 
      payload: { 
  ...
  // Handle gaps updates
  const handleGapsUpdated = useCallback((data: WsGapsUpdatedData) => {
  ...
  }, [dispatch, state.validationErrors, state.validationWarnings, state.isValidating, state.lastValidation]);

  // Handle IR updates
  const handleIrUpdated = useCallback((data: WsIrUpdatedData) => {
    dispatch({ 
      type: 'SET_IR', 
      payload: { 
        kind: data.kind, 
