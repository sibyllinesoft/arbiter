...
import { apiService, ApiService, ApiError } from '../api';
import type { 
  Project, 
  Fragment, 
...
describe('ApiService', () => {
  ...
  describe('IR endpoints', () => {
    ...
    it('should handle partial failures when getting all IRs', async () => {
      ...
      const mockCapabilities = { kind: 'capabilities', data: {}, generated_at: '2023-01-01T00:00:00Z' };
      ...
      mockFetch
        ...
        .mockResolvedValueOnce(createMockResponse({
          ok: true,
          status: 200,
          json: vi.fn().mockResolvedValue({ kind: 'coverage', data: {}, generated_at: '2023-01-01T00:00:00Z' }),
        }));
