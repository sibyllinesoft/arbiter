...
class MigrationRunner {
  ...
  async runMigrations(): Promise<void> {
    ...
    for (const migration of pending) {
      try {
        ...
        logger.error(`❌ Migration ${migration.id} failed`, error instanceof Error ? error : undefined);
        throw error;
      }
    }
  ...
  async rollbackMigration(id: string): Promise<void> {
    ...
    try {
      ...
      logger.error(`❌ Rollback of migration ${migration.id} failed`, error instanceof Error ? error : undefined);
      throw error;
    }
  }
