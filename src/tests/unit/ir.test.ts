...
describe("IRGenerator", () => {
  ...
  describe("Capabilities IR Generation", () => {
    it("should generate capabilities diagram data", async () => {
      ...
      const profileAuthEdge = edges.find(e => 
        e.source === "user.authentication" && e.target === "user.profile"
      );
  ...
  describe("Flows IR Generation", () => {
    it("should generate flow diagram data", async () => {
      ...
      const sequenceEdge = edges.find(e => 
        e.source === "user_onboarding.0" && e.target === "user_onboarding.1"
      );
  ...
  describe("Error Handling", () => {
    it("should throw error for unknown IR kind", async () => {
      ...
      await expect(
        generator.generateIR("unknown" as IRKind, resolved)
      ).rejects.toThrow("Unknown IR kind: unknown");
    });
      ...
      const resolved = {
        ...
        flows: {
          "flow1": { 
            ...
            steps: [
              { name: "Step 1", action: "action1", actor: "user" },
              { name: "Step 2", action: "action2", actor: "system", branches: [
                { condition: "branch1", name: "Branch 1" }
              ] }
            ]
          }
        },
