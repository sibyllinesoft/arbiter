...
import type { ServerConfig, Fragment, ValidationError } from "../../types.ts";
...
user: User & {
	...
	age:  -5 // Violates >= 0
      ...
      const constraintErrors = result.errors.filter(e => 
        e.message.includes("len") || e.message.includes("age")
      );
...
config: {
	port: 3000 // Conflict with above
      ...
      expect(result.success).toBe(false);
      expect(result.errors.length).toBeGreaterThan(0);
      expect(result.errors.some(e => e.message.includes("conflict") || e.message.includes("unification"))).toBe(true);
    });
  });
...
// No capabilities defined`,
      ...
      expect(result.errors.some(e => 
        e.type === "custom" && e.message.includes("capabilities")
      )).toBe(true);
    });
...
capabilities: {
	...
	"user.auth.login": {
      ...
      expect(result.warnings.some(w => 
        w.type === "duplicate" && w.message.includes("auth")
      )).toBe(true);
    });
  });
  ...
  describe("Gap Analysis", () => {
    ...
    it("should detect various token patterns", async () => {
      ...
      expect(tokenStrings.some(t => t.includes("user.firstName") || t.includes("user.lastName"))).toBe(true);
    });
  });
...
capabilities: {
    ...
    it("should handle filesystem errors gracefully", async () => {
      ...
      const invalidConfig = { ...testConfig, spec_workdir: "/invalid/readonly/path" };
