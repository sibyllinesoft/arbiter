...
describe("Concurrency and Race Condition Tests", () => {
  ...
  async function getResolvedSpec(projectId: string) {
    ...
    const resolvedResponse = await fetch(
      `${baseUrl}/api/resolved?project_id=${projectId}`
    );
  ...
  describe("Multi-Client Fragment Conflicts", () => {
    ...
    it("should handle simultaneous writes to same fragment with last-write-wins", async () => {
      ...
      console.log(`✅ Last-write-wins: Client ${winningClientIndex} won out of ${clientCount} concurrent writes`);
    });
    ...
    it("should prevent partial state corruption during concurrent validation", async () => {
      ...
      for (let i = 0; i < validationCount; i++) {
        ...
        // Either fully successful with valid spec_hash or failed with empty hash
        if (result.success) {
          expect(result.spec_hash.length).toBeGreaterThan(0);
          expect(result.resolved).toBeDefined();
        } else {
          expect(result.spec_hash).toBe("");
    ...
    it("should maintain fragment integrity under high concurrent load", async () => {
      ...
      const fragmentsResponse = await fetch(
        `${baseUrl}/api/fragments?project_id=${testProjectId}`
      );
      ...
      console.log(`✅ High load test: ${clientCount * fragmentsPerClient} fragments created by ${clientCount} clients in ${totalDuration}ms`);
    });
  });
  ...
  describe("Database Transaction Integrity", () => {
      ...
      for (let i = 0; i < operationCount; i++) {
        ...
        switch (operationType) {
          ...
          case 2: // List fragments (read operation)
            operations.push(
              fetch(`${baseUrl}/api/fragments?project_id=${testProjectId}`)
            );
            break;
        }
      }
    ...
    it("should maintain referential integrity during concurrent fragment operations", async () => {
      ...
      console.log(`✅ Referential integrity maintained across ${referenceCount} concurrent references`);
    });
  });
  ...
  describe("Validation Pipeline Race Conditions", () => {
    ...
    it("should handle concurrent fragment updates during active validation", async () => {
      ...
      // Spec hashes should be different (content changed)
    ...
    it("should maintain consistent spec hash computation under concurrency", async () => {
...
capabilities: {
  ...
  "project.management": "Project CRUD operations and collaboration"
      ...
      console.log(`✅ Spec hash consistency: ${hashes.length} concurrent validations produced identical hash: ${hashes[0]}`);
    });
  });
  ...
  describe("Resource Exhaustion and Recovery", () => {
    ...
    it("should gracefully handle resource exhaustion scenarios", async () => {
      ...
      for (let i = 0; i < heavyLoadCount; i++) {
        ...
        switch (operationType) {
          ...
          case 1: // Memory-intensive content
            const largeContent = `package spec\n\nlarge_data${i}: {\n` +
              Array.from({ length: 200 }, (_, j) => 
                `\tfield_${j}: "large_string_value_${'x'.repeat(50)}_${j}"`
              ).join('\n') + '\n}';
            operations.push(
          ...
          case 3: // IR generation (if fragments exist)
            if (i > 5) { // Only after some fragments exist
              operations.push(
      ...
      console.log(`✅ Resource exhaustion test: ${successful}/${results.length} operations succeeded in ${duration}ms`);
