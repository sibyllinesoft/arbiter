...
export async function handleGitHubPR(event: WebhookEvent): Promise<HandlerResponse> {
  try {
    ...
    const validationResult = validatePayload(event.payload, ['action', 'pull_request', 'repository']);
    ...
    if (action === 'opened' || action === 'synchronize') {
      ...
      if (!conventionalTitlePattern.test(pr.title)) {
        return createResponse(false, `PR title should follow conventional commit format. Current: "${pr.title}"`);
      }

      // Check if PR body is provided for non-draft PRs
      if (!pr.draft && (!pr.body || pr.body.trim().length < 10)) {
        return createResponse(false, 'PR description is required and should be at least 10 characters long');
      }

      // Validate target branch
      const allowedTargetBranches = ['main', 'master', 'develop', 'staging'];
      if (!allowedTargetBranches.includes(pr.base.ref)) {
        return createResponse(false, `Invalid target branch: ${pr.base.ref}. Allowed: ${allowedTargetBranches.join(', ')}`);
      }
      ...
      if (['main', 'master'].includes(pr.base.ref) && !sourceBranch.startsWith('hotfix/')) {
        if (!['develop', 'staging'].includes(pr.base.ref)) {
          return createResponse(false, 'Feature branches should target "develop" branch, not main/master directly');
        }
      }
    }
