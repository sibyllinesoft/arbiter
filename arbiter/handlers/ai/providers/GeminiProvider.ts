...
export class GeminiProvider implements AIProvider {
  ...
  private buildSystemInstructions(command: AICommand): string {
    const roleInstructions = {
      'review-code': `You are an expert software engineer specializing in code review. You have extensive experience in multiple programming languages, security analysis, performance optimization, and software architecture. Your role is to provide thorough, constructive feedback that helps developers improve their code quality.`,
      
      'analyze-issue': `You are a senior technical project manager with deep software development experience. You excel at understanding technical requirements, categorizing issues, assessing complexity, and providing clear guidance on issue resolution.`,
      
      'generate-docs': `You are a technical documentation specialist with expertise in creating clear, comprehensive, and developer-friendly documentation. You understand how to structure information for maximum clarity and usefulness.`,
      
      'security-scan': `You are a cybersecurity expert with specialized knowledge in application security, vulnerability assessment, and secure coding practices. You stay current with the latest security threats and mitigation strategies.`,
      
      'multimodal-analysis': `You are a technical analyst capable of processing multiple types of content including code, documentation, images, and other files. You can provide comprehensive analysis across different media types.`,
    };

    const baseInstruction = roleInstructions[command.name as keyof typeof roleInstructions] || 
...
Always provide specific examples and clear recommendations when possible.`;
  ...
  private buildEventContextSection(context: AIContext): string {
    ...
    let sections = [`## Repository Event Analysis`];
    ...
    if (eventData.pullRequest) {
      ...
      if (pr.body && pr.body.trim()) {
        ...
        sections.push('```');
        sections.push(pr.body.substring(0, 1000) + (pr.body.length > 1000 ? '\n... (truncated)' : ''));
        sections.push('```');
      }
      sections.push('');
    }
    ...
    if (eventData.issue) {
      ...
      sections.push(`- Assignees: ${issue.assignees.length > 0 ? issue.assignees.join(', ') : 'None'}`);
      
      if (issue.body && issue.body.trim()) {
        sections.push(`- Description:`);
        sections.push('```');
        sections.push(issue.body.substring(0, 1000) + (issue.body.length > 1000 ? '\n... (truncated)' : ''));
        sections.push('```');
      }
      sections.push('');
    }
...
Any automated actions to take`,
    ...
    return outputFormats[command.name as keyof typeof outputFormats] || 
  ...
  private extractActions(content: string, command: AICommand, context: AIContext): any[] {
    ...
    const actionSectionMatch = content.match(/🤖\s*Automated Actions?\s*\n(.*?)(?=\n#|$)/is) ||
    ...
    if (actionSectionMatch) {
      ...
      // Parse comments
      if (actionText.toLowerCase().includes('comment') || actionText.toLowerCase().includes('post')) {
  ...
  private parseCodeReview(content: string): any {
    const extractSection = (sectionName: string): string => {
      const regex = new RegExp(`###?\\s*[🔍🏗️🔒⚡📚✅]?\\s*${sectionName}[\\s\\n]([^#]*?)(?=\\n###?|$)`, 'is');
    ...
    const extractRating = (): string => {
      ...
      if (content.includes('🟠') || content.includes('Needs Improvement')) return 'needs-improvement';
      return 'good';
    };
    ...
    return {
      ...
      maintainability: {
        score: content.includes('excellent') ? 9 : content.includes('good') ? 7 : content.includes('poor') ? 3 : 6,
  ...
  private parseSecurityScan(content: string): any {
    return {
      ...
      vulnerabilities: this.extractBulletPoints(this.extractSection(content, 'Vulnerabilities|Security Issues')),
  ...
  private extractSection(content: string, sectionNames: string): string {
    ...
    for (const pattern of patterns) {
      const regex = new RegExp(`###?\\s*[🔍🏗️🔒⚡📚✅📋🎯🏷️👥📝📖✍️📁🔗]?\\s*${pattern}[\\s\\n]([^#]*?)(?=\\n###?|$)`, 'is');
      const match = content.match(regex);
